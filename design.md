...
## Battleground Tier Filters 2026-06-26

BG tier-list filters are a protected high-contrast control system:
- Filter panels, source toggles, race controls, tavern controls, and count pills must use cool dashboard surfaces: `#f8faff`, `#ebf1fc`, `#dbeafe`, `#1e3a8a`, `#2563eb`.
- Do not use dark brown, parchment gold, amber active states, or low-contrast blue-on-black combinations for BG filters.
- Active filter controls must be visibly blue/navy with white or near-white text; inactive controls must be light with dark slate text.
- Race and tavern filters must have explicit visible labels such as `Тип существа`, `Тип аксессуара`, and `Уровень таверны`; do not rely on icons alone.
- Tavern controls should use source `assset/tier*.png` icons from the BG site instead of plain numbers when those icons are available.
- Icon filters must be large enough to identify at a glance: at least 44px touch target, 28-32px icon, and accessible `title`/`alt` text.
- When filtering is client-side, keep data cached per list/source and avoid blank loading flashes on repeated tab switches.
